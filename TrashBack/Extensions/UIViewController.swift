...
extension UIViewController {
  ...
  public func setNavigationBarImage(for image: UIImage? = nil, color: UIColor = .white, requireShadowLine: Bool = false) {
      if let image = image {
    ...
    //MARK: BackButton
    public func setBackButton(tintColor: UIColor = .white, isImage: Bool = false, _ image: UIImage =  #imageLiteral(resourceName: "ic_BackButton") ) {
    ...
    //MARK: Like Button
    public func setRightButton(tintColor: UIColor = .white, isImage: Bool = false, image: UIImage =  #imageLiteral(resourceName: "ic_BackButton")){
        let btn1 = UIButton(type: .custom)
        if isImage {
            btn1.setImage(image.imageWithColor(color: tintColor), for: .normal)
